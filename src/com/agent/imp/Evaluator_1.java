...
public class Evaluator_1 implements Evaluator {
	...
	public int Utility(Piece[] state, Player p) {
		...
		return NumberOfPieces(p,state) - 
			   NumberOfPieces( (Player.BLACK==p?Player.RED:Player.BLACK)
					   			,state);
		/*if(p == Player.BLACK) {
			return NumberOfPieces(p,state)-NumberOfPieces(Player.RED,state);
		} else {
			return NumberOfPieces(Player.BLACK,state)-NumberOfPieces(p,state);
		}*/
	}
